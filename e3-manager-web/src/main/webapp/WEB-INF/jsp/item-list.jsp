...
<table class="easyui-datagrid" id="itemList" title="商品列表" 
       data-options="singleSelect:false,collapsible:true,fitColumns:true,striped:true,idField:'id',rownumbers:true,pagination:true,url:'/tb/item/list',method:'get',pageSize:30,toolbar:toolbar">
    <thead>
        <tr>
            ...
            <th data-options="field:'sellPoint',width:280">卖点</th>
        ...
        handler:function(){
        	...
        	$("#itemEditWindow").window({
        		onLoad :function(){
        			...
        			// 加载商品描述
        			$.getJSON('/tb/item/desc/'+data.id,function(_data){
        				if(_data.status == 200){
...
/*         			$.getJSON('/rest/item/param/item/query/'+data.id,function(_data){
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','确定删除ID为 '+ids+' 的商品吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
                	$.post("/tb/item/delete",params, function(data){
            			if(data.status == 200){
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','确定下架ID为 '+ids+' 的商品吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
                	$.post("/tb/item/instock",params, function(data){
            			if(data.status == 200){
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','确定上架ID为 '+ids+' 的商品吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
                	$.post("/tb/item/reshelf",params, function(data){
            			if(data.status == 200){
